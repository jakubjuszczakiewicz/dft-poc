...
void dft_avx2(double * output, double * input, double * costab, uint32_t fft_size);
...
int main(int argc, char * argv[])
{
  ...
  double * out4 = malloc(sizeof(double) * FFT_SIZE);
  ...
  dft_avx2(out4, in, coss, FFT_SIZE);
  fftw_execute(p);

  uint64_t t_dft = 0, t_sse = 0, t_fftw = 0, t_avx = 0;
  ...
  for (size_t i = 0; i < FFT_SIZE; i++) {
    ...
    if (out[i] != out4[i])
      printf("avx: %zu %f %f %e\n", i, out[i], out4[i], fabs(out[i] - out4[i]));
  }
